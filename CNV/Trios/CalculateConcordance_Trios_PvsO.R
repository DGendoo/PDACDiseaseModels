...
for (count in 1:length(CNVList))
{
  ...
  for(counter in 1:length(chromList)) #For every chromosome
  {
    ...
    TumourSub<-Sample[,c("TuChr","TuStart","TuEnd","TuSample","TuCN")] #First get only the tumour data
    ...
    PDOSub<-Sample[,c("PDOChr","PDOStart","PDOEnd","PDOSample","PDOCN")] #First get only the tumour data
    ...
    ConcordanceRateByChr[[counter]]<-Intersecting_Bases/(All_Tumour_Bases + All_PDO_Bases - All_Common_Bases)
...
write.csv(FullMatrix,file="Aug2018_CNV_Jaccard_PDAC_Trios_TumourVsPDO.csv")
save.image(file="Aug2018_CN_Concordance_PDAC.RData") #Image last run on August 10, 2018
