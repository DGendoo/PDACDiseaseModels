...
library(gplots)
pdf("Aug2018_Jaccard_By_Chr_LIVER_Heatmap.pdf",height = 10,width = 10)
heatmap.2(as.matrix((Jaccard_By_Chr2)),Rowv = F,Colv = F,trace = "none",na.color = "white",
          col = c('#543005','#8c510a','#dfc27d','#c7eae5','#35978f'))
dev.off()

write.csv(round(Jaccard_By_Chr2,4),file="Aug2018_Jaccard_By_Chr_LIVER.csv")


library(gplots)
pdf("Aug2018_Jaccard_By_Chr_LIVER_Heatmap2.pdf",height = 12,width = 4)
heatmap.2(as.matrix((Jaccard_By_Chr2)),Rowv = F,Colv = F,trace = "none",na.color = "white",
...
save.image("Aug2018_Jaccard_Calculations_LIVER.RData")

######################################################################################################
######################################################################################################
######################################################################################################
# CALCULATE EVENTS BY CATEGORY OF THE STRUCTURAL VARIATION
...
Tumour_TSVList_ByCategory<-data.frame(matrix(unlist(Tumour_TSVList_ByCategory), nrow=6, byrow=T))
...
Xeno_TSVList_ByCategory<-data.frame(matrix(unlist(Xeno_TSVList_ByCategory), nrow=6, byrow=T))
...
Common_TSVList_ByCategory<-data.frame(matrix(unlist(Common_TSVList_ByCategory), nrow=6, byrow=T))
...
Jaccard_By_Cat2<- data.frame(matrix(unlist(Jaccard_By_Cat), ncol=6, byrow=F))
